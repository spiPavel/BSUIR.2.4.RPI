...
function* getPermutations(chars) {
    yield* permutation(chars.split(''), 0, chars.length - 1);

    function* permutation(characters, i, count) {
        if (i === count) {
            yield characters.join('');
        } else {
            for (let j = i; j <= count; j++) {
                let charBuffer = characters[i];
                characters[i] = characters[j];
                characters[j] = charBuffer;

                yield* permutation(characters, i + 1, count);

                charBuffer = characters[i];
                characters[i] = characters[j];
                characters[j] = charBuffer;
            }
        }
...
function getMostProfitFromStockQuotes(quotes) {
    ...
    quotes.forEach((value, index) => {
        maxProfit += quotes.slice(index).sort((a, b) => b - a)[0] - value;
    });
    return maxProfit;
}
